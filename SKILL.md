...
- 同一 `Dialog` 服务打开多层对话框时会共享一个遮罩，并按打开栈统一管理层级；遮罩点击只作用于最上层参与遮罩的对话框。不要按“每个 Dialog 都有独立遮罩”编写选择器、计数或层级逻辑，也不要手工覆盖各层 host 的 `z-index`。`hasBackdrop: false` 仅表示该层不参与共享遮罩，不会移除其他已打开对话框所需的遮罩。
